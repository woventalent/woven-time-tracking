...
- **Projects** — Auto-generated project codes (workspace-configurable prefix, e.g. WRI-001, WRI-002, …), project types with color tags, budgeted hours with usage progress bar, file attachments, and team member allocation (with a designated SPOC per project); assigned members are emailed when added to a project. The list view is deliberately lean — Code, Project Name, Type, Request Date, Client, Users Assigned, Status, and TAT (turnaround time in business days between Report Initiated and Report Delivered) — and sorts by Request Date (newest first) by default. A "Users Assigned" column shows project members as soon as they're assigned, regardless of whether they've logged time yet, visible to admins and members alike without scrolling. Requestor, Credits, Report Initiated, and Report Delivered move to the project detail panel (opened by clicking a row) alongside Members/Timesheets/Documents tabs. Admins can export the full project list (including Requestor and Credits) to CSV.
- **Timesheets** — Log time against your assigned projects only; entries are tied to the logged-in user automatically and can only be edited/deleted by their owner (or a workspace admin). The Log Time project dropdown only lists active projects for the selected client (an in-progress entry's own project always stays selectable even if since marked inactive). Log Time defaults to the project's Report Initiated date and is constrained to fall within the project's Report Initiated / Report Delivered window (enforced both client- and server-side)
...
- **Reports** — Time by user, project, client, or period (By Period, with Daily / Weekly / Monthly grouping), with budget vs. logged hours visualization, CSV export, and a report delivery calendar view showing delivered/upcoming report dates; "Users Assigned" reflects project membership, not just users who've logged time
