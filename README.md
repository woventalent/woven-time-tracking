...
- **Timesheets** — Log time against your assigned projects only; entries are tied to the logged-in user automatically and can only be edited/deleted by their owner (or a workspace admin). The Log Time project picker only lists active projects (an in-progress edit keeps its own project selectable even if that project has since been put on hold/completed/cancelled). Log Time defaults to the project's Report Initiated date and is constrained to fall within the project's Report Initiated / Report Delivered window (enforced both client- and server-side)
