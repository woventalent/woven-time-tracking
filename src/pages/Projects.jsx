        ...
        <table style={{ width: '100%', minWidth: 1000, borderCollapse: 'collapse', fontSize: 13.5 }}>
          <thead>
            <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
              {['Code', 'Project Name', 'Type', 'Request Date', 'Client', 'Users Assigned', 'Status', 'TAT (days)', ''].map(h => {
                ...
                >
                  ...
                  <td style={{ padding: '13px 16px', whiteSpace: 'nowrap' }}>
                    {(() => {
                      ...
                      return <span style={{ color: tat != null ? '#0f172a' : '#cbd5e1', fontWeight: tat != null ? 600 : 400 }}>{tat != null ? `${tat}d` : '—'}</span>
                    })()}
                  </td>
                  <td style={{ padding: '13px 16px', whiteSpace: 'nowrap' }}>
      ...
      <div style={{ width: 520, height: '100vh', background: '#fff', display: 'flex', flexDirection: 'column', boxShadow: '-4px 0 32px rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ padding: '22px 24px 16px', borderBottom: '1px solid #e2e8f0' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 16 }}>
            ...
            <DetailField label="TAT"              value={(() => { const t = businessDays(project.report_initiated, project.report_delivered); return t != null ? `${t} business days` : null })()} />
          </div>
...
function DetailField({ label, value }) {
  return (
    <div>
      <div style={{ fontSize: 10.5, color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>{label}</div>
      <div style={{ fontSize: 13, color: value ? '#1e293b' : '#cbd5e1', fontWeight: 500 }}>{value || '—'}</div>
    </div>
  )
}
